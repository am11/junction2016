...
public struct MapCoordinates
{
	...
	{
		string[] tuple = (raw ?? string.Empty).Split(';');

		return tuple.Length == 3 ? new MapCoordinates(float.Parse(tuple[0]), float.Parse(tuple[1]), double.Parse(tuple[2])) : new MapCoordinates();
	}
